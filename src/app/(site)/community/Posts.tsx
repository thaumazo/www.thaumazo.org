...
export default async function Posts() {
  const postData = await listMD("community/people");
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 max-w-3xl mx-auto my-8">
      {posts.map(({ title, image, slug }) => {
        ...
        return (
          <div key={slug} className="flex justify-center flex-col gap-1">
            <Link
              className="mx-auto flex items-center justify-center w-32 h-32 rounded overflow-hidden bg-gray-200 dark:bg-gray-800"
              href={link}
            >
              {image ? (
                <Image
